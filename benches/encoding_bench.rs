...
fn bench_comparison(c: &mut Criterion) {
    ...
    for size in sizes.iter() {
        ...
        if should_run("turbo") {
            group.bench_with_input(BenchmarkId::new("Decode/Turbo", size), encoded_str.as_bytes(), |b, d| {
                b.iter(|| {
                    ...
                    unsafe {
                        let len = decode_slice_unsafe(black_box(d), &mut buffer).unwrap();
                        buffer.set_len(len);
                    };
        ...
        // // 3. b58 Standard
        // if should_run("b58")  {
        //     group.bench_with_input(BenchmarkId::new("Decode/b58", size), &encoded_str, |b, d| {
        //         b.iter(|| decode_b58(black_box(d)).unwrap())
        //     });
        // }
        ...
        // // 5. vbs58 "SIMD"
        // if should_run("vbs58") {
        //     group.bench_with_input(BenchmarkId::new("Decode/vbs58", size), &encoded_str, |b, d| {
        //         b.iter(|| decode_simd(black_box(d)).unwrap())
        //     });
        // }
